...
The site must load fast on mid-range Android phones over 4G (Lighthouse mobile ≥ 90 performance), respect accessibility preferences, correctly handle all external deep-links, and run entirely within free-tier limits. Security must ensure public users can only read data (except session registrations), admin mutations are protected, and secrets never leak to the client bundle. The final deployment must be documented in plain language for non-technical organizers who will need to update external URLs, contact details, and payment information post-launch.
...
| Table | Public SELECT | Public INSERT | Admin UPDATE/DELETE |
|---|---|---|---|
| `tournaments` | Yes (where `registration_open = true`) | No | Yes |
| `sessions` | Yes | No | Yes |
| `session_registrations` | No | Yes (rate-limited) | Yes |
| `products` | Yes (where `active = true`) | No | Yes |
| `product_images` | Yes | No | Yes |
| `sponsors` | Yes (where `active = true`) | No | Yes |
| `contact_settings` | Yes | No | Yes |
| `site_settings` | Yes | No | Yes |

**Rate Limiting:**
- Add basic rate-limiting to `session_registrations` insert
- Implementation: IP + timestamp check in Server Action (simple in-memory or Vercel's built-in protections)
- Target: max 5 submissions per IP per hour
...
3. Create Storage buckets: `payment-proofs`, `tournament-posters`, `product-images`, `sponsor-logos`
...
4. **Where to Paste External URLs** — tournament registration form link, shop order form link
5. **Where to Update Payment Details** — Maybank account info, TouchNGo QR image (in admin Site Settings)
6. **Where to Update Contact Info** — WhatsApp number, email address, Instagram handle (in admin Contact Settings)
7. **Deploy Process** — "Push code to GitHub, Vercel auto-deploys"
8. **Environment Variables** — list of all env vars and where they're used
...
| Click "Join this Session" | Multi-step form opens |
| Submit registration with payment proof | Confirmation screen, registration in Supabase |
...
| 11 | Rate limiting on session_registrations | Submit 6+ registrations, verify rejection |
| 12 | File uploads validated server-side | Try invalid file type via curl |
| 13 | Service role key server-only | Grep verification |
| 14 | No hardcoded hex/copy outside allowed locations | Grep verification |
| 15 | Production deploy succeeds on Vercel | Check Vercel dashboard |
| 16 | README.md exists with plain-language docs | Check file contents |
...
- The site will not exceed free-tier limits at current scale (400 members, ~200 registrations/quarter)
